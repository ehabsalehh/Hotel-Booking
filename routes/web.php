...
Route::group(['middleware' => ['auth', 'checkRole:Super,Admin']], function () {
    ...
    Route::name('transaction.reservation.')->group(function () {
        Route::controller(TransactionRoomReservationController::class)->group(function(){
            Route::get('/create-identity', 'createIdentity')->name('createIdentity');
            Route::get('/view-count-person/{customer}', 'viewCountPerson')->name('viewCountPerson');
            Route::get('/pick-from-customer', 'pickFromCustomer')->name('pickFromCustomer');
            Route::post('/store-customer', 'storeCustomer')->name('storeCustomer');
            Route::get('/choose-room/{customer}', 'chooseRoom')->name('chooseRoom');
            Route::get('/confirmation', 'confirmation')->name('confirmation');
            Route::post('/pay-down-payment', 'payDownPayment')->name('payDownPayment');
        });
    });
    ...
    Route::get('/get-daily-guest-chart-data', [ChartController::class, 'dailyGuestPerMonth']);
    Route::get('/get-daily-guest/{year}/{month}/{day}', [ChartController::class, 'dailyGuest'])->name('chart.dailyGuest');
});
