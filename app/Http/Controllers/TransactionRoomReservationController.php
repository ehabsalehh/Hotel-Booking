...
class TransactionRoomReservationController extends Controller
{
    ...
    public function storeCustomer(StoreCustomerRequest $request, CustomerRepository $customerRepository)
    {
        ...
        return redirect()->route('transaction.reservation.viewCountPerson',
    ...
    public function chooseRoom(ChooseRoomRequest $request, Customer $customer)
    {
        ...
        $rooms = $this->reservationRepository->getUnoccupiedRoom($request, $occupiedRoomId);

        $roomsCount = $this->reservationRepository->countUnoccupiedRoom($request, $occupiedRoomId);
        return view('transaction.reservation.chooseRoom', compact('customer', 'rooms', 'CheckinDate', 'CheckoutDate', 'roomsCount'));
    }
    public function confirmation( Request $request)
    {
        $room = Room::findOrFail($request->room);
        $customer = Customer::findOrFail($request->customer);
        $CheckinDate = $request->from;
        $CheckoutDate = $request->to;
        $price = $room->price;
        $dayDifference = Helper::getDateDifference($CheckinDate, $CheckoutDate);
            ...
            PaymentRepository $paymentRepository)
    {
        ...
        if (in_array($room->id, $occupiedRoomIdInArray)) {
            return redirect()
                    ->back()
                    ->with('failed', 'Sorry, room ' . $room->number . ' already occupied');
        }
    ...
    private function getOccupiedRoomID($CheckinDate, $CheckoutDate)
    {
        $occupiedRoomId = Transaction::where([
